...
if (typeof ComponentRegistry !== 'undefined') {
    ComponentRegistry.register('double-card-slider', {
        mounted(element, data) {
            new DoubleCardSliderCarousel(element.querySelectorAll('[data-carousel]')[0]);
        }
    });
}
...
(function() {
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.double-card-slider [data-carousel]').forEach(carousel => new DoubleCardSliderCarousel(carousel));
    });
})();

class DoubleCardSliderCarousel {
    constructor(carousel) {
        this.carousel = carousel;
        this.slidesContainer = carousel.querySelector('[data-carousel-slides-container]');
        if (!this.slidesContainer) {
            console.warn('DoubleCardSlider: Missing [data-carousel-slides-container] element');
            return;
        }
        this.paginationContainer = carousel.querySelector('[data-carousel-pagination]');
        if (!this.paginationContainer) {
            console.warn('DobuleCardSlider: Missing [data-carousel-pagination] element');
            return;
        }
        this.currentSlide = 0;
    ...
    createDots() {
        ...
        for (let i = 0; i < this.numSlides; i++) {
            ...
            dot.classList.add('double-carousel-dot');
    ...
    handleDotClick(event) {
        if (event.target.classList.contains('double-carousel-dot')) {
