...
if (typeof ComponentRegistry !== 'undefined') {
    ComponentRegistry.register('hero-technology', {
        mounted(element, data) {
            new HeroTechnologyCarousel(element.querySelectorAll('[data-carousel]')[0]);
        }
    });
}
...
(function() {
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.hero-technology [data-carousel]').forEach(carousel => new HeroTechnologyCarousel(carousel));
    });
})();

class HeroTechnologyCarousel {
    constructor(carousel) {
        ...
        if (!this.slidesContainer) {
            console.warn('Missing [data-carousel-slides-container] element in hero-technology carousel');
            return;
        }
        if (!this.paginationContainer) {
            console.warn('Missing [data-carousel-pagination] element in hero-technology carousel');
            return;
        }
        this.currentSlide = 0;
